		...
		div.innerHTML=`
			...
			<div class="goods-desc">
				<a href="javascript:void(0);">
					<span>${product.tit}</span>
				</a>
			</div>
			<!--price-box价格盒子  -->
			<div class="price-box fr">
				<div class="item-desc good-price">
					<span>${product.price}</span>元&nbsp;x
					<span class="txt">${product.num}</span>
				</div>
				<div class="item-desc good-status"></div>
				<!-- 总计 -->
				<div class="item-desc good-total">
					<span>${product.price*product.num}</span>元
				</div>
			</div>
		`
		document.querySelector('.goods-list').appendChild(div);
	})
}
...
function AddrBtn() {
	...
	//2为创建的元素添加内容
	div.innerHTML =
				`<div class="bor-color address-list">
					<div class="address-info">
						<div class="name">
							<span>
								${Name.value}
...
var borColor=document.querySelector('.bor-color');
var settlementBtn=document.querySelector('.btn-settlement')
borColor.onclick=function(){
	if(borColor.style.borColor!="#ff6700"){
		borColor.style.borderColor="#ff6700";
		settlementBtn.innerText="立即下单";
		
	}else{
		borColor.style.borderColor="yellow";
		settlementBtn.innerText="去结算";
	}
}
settlementBtn.onclick=function(){
	if(settlementBtn.innerText=="去结算"){
		alert('请选择地址！');
		return false;
	}
}